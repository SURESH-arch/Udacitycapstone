		...
		stage('Build Docker Image') {
			steps {
				withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'dockerhub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
					sh '''
						docker build -t sureshgtechno/udacity-capstone .
					'''
				}
			}
		}
			...
			steps {
				withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'dockerhub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
					sh '''
						...
						docker push sureshgtechno/udacity-capstone
					'''
				}
			}
		}
		...
		stage('Set Current kubectl Context') {
			steps {
				withAWS(region:'us-east-2', credentials:'aws-id') {
					sh '''
						kubectl config use-context arn:aws:eks:us-east-2:821556368364:cluster/udacitycluster
					'''
				}
			}
		}
